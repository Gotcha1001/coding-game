...
export default function RentalOffice() {
    ...
    const rentApartment = (tier) => {
        let rentAmount, deposit, apartmentTier;
        switch (tier) {
            case 'basic':
                rentAmount = 50;
                deposit = 100;
        ...
        dispatch({
            type: 'RENT_APARTMENT',
            payload: { rentAmount, deposit, apartmentTier },
        });
        dispatch({ type: 'USE_TIME', payload: { amount: 15 } });
        setShowRentModal(false);
        showMessage(`You've successfully rented a ${apartmentTier} apartment!`);
    };
            ...
            <div className="relative">
                <div className="absolute top-0 left-0 w-full h-full bg-gray-900 z-0">
                    <video
                        ...
                        muted
                        playsInline
                        className="absolute top-0 left-0 w-full h-full object-cover object-[center_top] z-0 opacity-100"
                    >
                        <source src="https://cdn.pixabay.com/video/2022/08/27/129247-743827327_tiny.mp4" type="video/mp4" />
