...
//         dispatch({ type: "RESET_GAME_WON" }); // Reset gameWon to allow screen change
//     };
...
//                                 <p className={`text-lg ${hasMasteredOneSubject ? "text-green-400" : "text-red-400"}`}>
...
import confetti from "canvas-confetti";
import { motion } from "framer-motion";
import { winConditions } from "@/data/winConditions"; // Import winConditions
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogContent,
...
export default function GameOver({ won }) {
    const { state, dispatch } = useGame();
    const { player, gameWon } = state;
    const hasFiredConfetti = useRef(false); // Track if confetti has fired

    // Function to count mastered subjects (consistent with winConditions.js)
    const countMasteredSubjects = () => {
        let subjectsMastered = 0;
        subjectsMastered += Object.values(player.hooksMastered || {}).filter((v) => v >= 1).length;
        subjectsMastered += Object.values(player.nextJsMastered || {}).filter((v) => v >= 1).length;
    ...
    // Check if conditions are met using imported winConditions
    const conditionsMet = {
